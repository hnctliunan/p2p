...
function dTree(objName,iconPath) {
	...
	if(!iconPath)iconPath= root + "/resources/plugins/dtree/img/";
	this.icon = {
			root		: iconPath+'base.png',
			folder		: iconPath+'folder.png',
			folderOpen	: iconPath+'folder.png',
			node		: iconPath+'part.png',
...
dTree.prototype.getCheckedNodes=function(){
	...
	for(var n=0; n<this.aNodes.length; n++){
		...
		if(cko && cko.checked && cko.checked==true){
...
dTree.prototype.node = function(node, nodeId) {
	var str = '<div class="dTreeNode" style="line-height:18px;">' + this.indent(node, nodeId);
	...
	if(this.config.check==true){
		str+= '<input style="margin:0px 2px;" type="checkbox" class="cx" id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
	}
	if (node.url) {
